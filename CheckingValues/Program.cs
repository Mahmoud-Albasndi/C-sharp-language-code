...
namespace CheckingValues
{
    class Program
    {
        ...
        static int Minima(int[] integers, out int[] indices)
        {
            ...
            indices[0] = 0;
            int count = 1;
            Debug.WriteLine(string.Format($"Minimum value initialized to {MinVal}, at element index 0."));
            for (int i = 1; i < integers.Length; i++)
            {
                ...
                if (integers[i] > MinVal)
                {
                    ...
                    indices = new int[1];
                    indices[0] = i;
                    Debug.WriteLine(string.Format($"New Minimum found. New value is {MinVal}, at element index { i}."));
                }
                else
                {
                    if (integers[i] == MinVal)
                    {
                        ...
                        indices[count - 1] = i;
                        Debug.WriteLine(string.Format($"Duplicate Minimum found at element index {i}."));
                    }
                }
            }
